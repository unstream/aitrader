...
import net.unstream.aitrader.strategies.*;
...
public class AITrader {
	private void runDax() throws IOException {
		...
		File outFile = new File("src/main/resources/KI2Strategy.csv");
		outFile.createNewFile();
		BufferedWriter out = new BufferedWriter(new FileWriter(outFile));

		CSVPrinter csvPrinter = new CSVPrinter(out, CSVFormat.DEFAULT.withHeader("time", "balance", "dax"));

		Account myAccount = Account.builder().cash(1000).strategy(new KI2Strategy()).build();
